...
def create_rpn_formula(next_numbers, previous_formula=[]):
    if previous_formula == []:
        formula = [next_numbers[0]]
        next_numbers = next_numbers.copy()
        del next_numbers[0]

        return create_rpn_formula(next_numbers, formula)
    ...
    if len(next_numbers) > 0:
        ...
        for formula in formulas:
            next_formulas += create_rpn_formula(next_numbers, formula)
...
def main():
    ...
    for line in yield_lines("2024/day7/input"):
        ...
        formulas = create_rpn_formula(numbers)
